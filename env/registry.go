...
func (env *WinEnv) ExportUserVar(opt EnvVarExportOpt) *WinEnv {
	file := opt.File
	if len(file) == 0 {
		file = fmt.Sprintf("user_%s.ini", util.NowTimestampByString())
	}
...
func (env *WinEnv) ExportSysVar(opt EnvVarExportOpt) *WinEnv {
	file := opt.File
	if len(file) == 0 {
		file = fmt.Sprintf("sys_%s.ini", util.NowTimestampByString())
	}
	conf := NewRegistryValueFile(file)
...
func (env *WinEnv) DeleteUserVar(opt EnvVarDeleteOpt) *WinEnv {
	fp := NewRegistryValueFile(fmt.Sprintf("%s.ini", util.NowTimestampByString()))
...
func (page *RegistryPage) Backup() error {
	if err := util.Mkdirs(page.path); err != nil {
		return err
	}
